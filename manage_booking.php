    ...
    if (!$isLoggedIn) {
        ...
        if (empty($paymentMethod) || !in_array($paymentMethod, ['Cash', 'GCash', 'Bank Transfer'])) {
            ...
            if (empty($cardNumber) || empty($cardholderName)) {
                $message = 'Please provide card number and cardholder name for bank transfer.';
                $messageType = 'danger';
            } elseif (!preg_match('/^\d{16}$/', preg_replace('/\s+/', '', $cardNumber))) {
                $message = 'Please enter a valid 16-digit card number.';
                $messageType = 'danger';
            } else if (empty($eventType) || empty($eventDate) || empty($eventLocation) || $numberOfGuests < 1) {
                $message = 'Please fill in all required fields.';
                $messageType = 'danger';
            } else {
                goto validation_passed; // Skip to next validation
            }
...
if (empty($eventType) || empty($eventDate) || empty($eventLocation) || $numberOfGuests < 1) {
    ...
    if ($totalAmount <= 0) {
        ...
        // Build special requests with payment method
        ...
        if ($paymentMethod === 'Bank Transfer') {
            ...
            if (empty($cardNumber) || empty($cardholderName)) {
                ...
                $fullSpecialRequests .= "Bank Transfer Details:\n";
                $fullSpecialRequests .= "Cardholder: $cardholderName\n";
                $fullSpecialRequests .= "Card (last 4): " . substr(preg_replace('/\s+/', '', $cardNumber), -4) . "\n\n";
                $fullSpecialRequests .= $specialRequests;
                
                // Proceed to insert booking (continue with your existing code)
            }
        } else {
            $fullSpecialRequests .= $specialRequests;
            // Proceed to insert booking (continue with your existing code)
            ...
            try {
                ...
                $insertBooking->bind_param("isssisd", $client_id, $eventType, $eventDate, $eventLocation, $numberOfGuests, $fullSpecialRequests, $totalAmount);
    ...
    // Store GCash payment details in session for modal display
    $_SESSION['show_gcash_modal'] = true;
    $_SESSION['gcash_amount'] = $totalAmount;
    $_SESSION['gcash_booking_id'] = $bookingID;
    
    $message = "Booking submitted successfully! Your booking ID is #$bookingID. Please scan the QR code to complete payment.";
    $messageType = 'success';
} else if ($paymentMethod === 'Bank Transfer') {
    $message = "Booking submitted successfully! Your booking ID is #$bookingID. Total amount: ₱" . number_format($totalAmount, 2) . ". Bank transfer details saved. We will review your request and send you a quotation shortly.";
    $messageType = 'success';
} else {
        ...
        document.getElementById('bookingForm').addEventListener('submit', function(e) {
            ...
            if (selectedPaymentMethod === 'Bank Transfer') {
                ...
                if (!cardNumber || !cardholderName) {
                    e.preventDefault();
                    alert('Please fill in all card details (Card Number and Cardholder Name)');
                    return false;
                }
